(function() {
  ...
  d3.timeline = function() {
    ...
    function timeline (gParent) {
          ...
          d.forEach(function (datum, index) {
            ...
            if (ending == 0 && beginning == 0){
              /*datum.times.forEach(function (time, i) {
                if (time.startTime < minTime || minTime == 0)
                  minTime = time.startTime;
                if (time.endTime > maxTime)
                  maxTime = time.endTime;
              });*/
              if (datum.startTime < minTime || minTime == 0)
                minTime = datum.startTime;
              if (datum.endTime > maxTime)
                maxTime = datum.endTime;
      ...
      var scaleFactor = (.25/(ending - beginning)) * (width - margin.left - margin.right);
      // draw the axis
      ...
      g.each(function(d, i) {
        d.forEach( function(datum, index){
          var data = [{"name":datum.name,"startTime":datum.startTime,"endTime":datum.endTime}];
          var hasLabel = (typeof(datum.eventId) != "undefined");
          ...
          g.selectAll("svg").data(data).enter()
            ...
            .attr("width", function (d, i) {
              return (d.endTime - d.startTime) * scaleFactor;
            })
      ...
      function getXPos(d, i) {
        return margin.left + (d.startTime - beginning) * scaleFactor;
      }
